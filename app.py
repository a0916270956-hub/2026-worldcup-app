...
# ==========================================
# 3. UI 模組：絕對像素畫線與置中系統 (究極進化版)
# ==========================================
def get_connectors(count, height):
    """純 CSS 數學鎖定畫線：精準匯聚前一輪的上下兩張卡片"""
    html = '<div style="display: flex; flex-direction: column; width: 15px;">'
    for _ in range(count):
        # 繪製 "]" 的形狀
        html += f'''
        <div style="height: {height}px; display: flex; align-items: center; justify-content: flex-start;">
            <div style="width: 100%; height: {height // 2}px; border-top: 2px solid #bdc1c6; border-bottom: 2px solid #bdc1c6; border-right: 2px solid #bdc1c6; box-sizing: border-box;"></div>
        </div>
        '''
    ...
    for _ in range(count):
        ...
        html += f'''
        <div style="height: {height}px; display: flex; align-items: center; justify-content: flex-start;">
            <div style="width: 100%; height: 0px; border-top: 2px solid #bdc1c6;"></div>
        </div>
        '''
    html += '</div>'
    return html
...
if "error" in matches_result:
    ...
    with tab1:
        ...
        st.caption("💡 提示：介面已套用純粹 CSS 數學定位，保證線條 100% 準確無誤地連接卡片，且免疫任何螢幕變形！請左右滑動檢視。")
        ...
        def build_col(matches, height):
            res = ""
            for m in matches:
                res += f'<div style="height: {height}px; display: flex; align-items: center; justify-content: center;">{get_match_card_html(m)}</div>'
            return res
        ...
        bracket_html = (
            ...
            '</div>'
            # 對戰網格矩陣本體 (高度寫死為 90px * 16場 = 1440px)
            ...
            # 將冠軍賽幾何中心絕對鎖定在整個矩陣的中心 (1440 / 2 = 720px)
