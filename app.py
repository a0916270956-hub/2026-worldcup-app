...
# 淘汰賽資料 (因對戰組合尚未出爐，先以代號表示)
knockout_matches = {
    "🔥 決賽 & 季軍戰": [
        "7/20 03:00 [冠軍戰] 準決賽勝者1 VS 準決賽勝者2",
        "7/19 05:00 [季軍戰] 準決賽敗者1 VS 準決賽敗者2"
    ],
    "⭐ 4強賽 (共2場)": ["7/15 待定 [4強賽1]", "7/16 待定 [4強賽2]"],
    "⚔️ 8強賽 (共4場)": [f"7/10-7/12 待定 [8強賽 {i+1}]" for i in range(4)],
    "🎯 16強賽 (共8場)": [f"7/4-7/7 待定 [16強賽 {i+1}]" for i in range(8)],
    "🚀 32強賽 (共16場)": [f"6/29-7/3 待定 [32強賽 {i+1}]" for i in range(16)]
}
...
with tab3:
    ...
    new_score = st.text_input("3. 輸入比分 (例如： 2 - 1)", value=current_score)
